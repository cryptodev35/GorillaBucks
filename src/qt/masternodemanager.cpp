...
void MasternodeManager::on_localButton_clicked()
{
    ...
    // Check if a local Gorilla Band Node already exists
    BOOST_FOREACH(PAIRTYPE(std::string, CAdrenalineNodeConfig) adrenaline, pwalletMain->mapMyAdrenalineNodes)
    {
        if(adrenaline.second.isLocal)
	{
    ...
    {
	QMessageBox msg;
        msg.setText("A local Gorilla Band Node already exists.");
	msg.exec();
	return;
    }
    ...
    {
	QMessageBox msg;
        msg.setText("You must have at least 5000.1 BUCKS to cover the 5000 BUCKS collateral for a Gorilla Band Node and the tx fee.");
	msg.exec();
	return;
    }
    ...
    {
	QMessageBox msg;
        msg.setText("Your wallet must be unlocked so that the 5000 BUCKS collateral can be sent.");
	msg.exec();
	return;
    }
    ...
    {
	QMessageBox msg;
	std::string sMsg = "Local Gorilla Band Node created and 5000 BUCKS sent to the collateral address.  Transaction hash:\n";
