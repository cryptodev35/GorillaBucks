...
void Shutdown()
{
    ...
    RenameThread("GorillaBucks-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: GorillaBucks.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: GorillaBucksd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeGorillaBucksamount=<n> " + _("Keep N GorillaBucks anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GorillaBucks is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GorillaBucks is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("GorillaBucks version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeGorillaBucksAmount = GetArg("-anonymizeGorillaBucksamount", 0);
    if(nAnonymizeGorillaBucksAmount > 999999) nAnonymizeGorillaBucksAmount = 999999;
    if(nAnonymizeGorillaBucksAmount < 2) nAnonymizeGorillaBucksAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeGorillaBucksAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize GorillaBucks Amount %d\n", nAnonymizeGorillaBucksAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1GorillaBucks+1000 == (.1GorillaBucks+100)*10
       10GorillaBucks+10000 == (1GorillaBucks+1000)*10
    */
